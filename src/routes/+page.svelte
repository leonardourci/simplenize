<script lang="ts">
  const transformationOptions = ['Para CAIXA ALTA', 'Parágrafos para uma linha', 'Capitalizar Texto'];
  ...
  $: {
    ...
    if (selectedTransformations.includes('Para CAIXA ALTA')) {
      output = output.toUpperCase();
    }
    if (selectedTransformations.includes('Capitalizar Texto')) {
      const result: string[] = []
...
<div class="min-h-screen bg-gradient-to-br from-blue-100 to-purple-100">
  <header class="w-full h-52 bg-white shadow-md">
    <div>
      <img class="mx-auto" width="220" src="/Conversia-logo-removebg-preview.png" alt="Conversia" />
    </div>
  </header>
